...
class LateralUpsampleMerge(nn.Module):
    ...
    def forward(self, x):
        return self.conv_lat(self.hook.stored) + F.interpolate(
            x, self.hook.stored.shape[-2:], mode="nearest"
        )
...
class RetinaNet(nn.Module):
    ...
    def _head_subnet(self, n_classes, n_anchors, final_bias=0.0, n_conv=4, chs=256):
        ...
        layers = [
            conv_layer(chs, chs, bias=True, norm_type=None) for _ in range(n_conv)
        ]
...
class RetinaNetFocalLoss(nn.Module):
    ...
    def _one_loss(self, clas_pred, bbox_pred, clas_tgt, bbox_tgt):
        ...
        return bb_loss + self._focal_loss(clas_pred, clas_tgt) / torch.clamp(
    ...
    def forward(self, output, bbox_tgts, clas_tgts):
        ...
        self.n_classes = clas_preds.size(2)
        return sum(
            [
...
def create_grid(size):
    ...
    linear_points = torch.linspace(-1 + 1 / H, 1 - 1 / H, H) if H > 1 else tensor([0.0])
...
def activ_to_bbox(acts, anchors, flatten=True):
    ...
    if flatten:
        acts.mul_(
            acts.new_tensor([[0.1, 0.1, 0.2, 0.2]])
...
def process_output(output, i, detect_thresh=0.8):
    ...
    if clas_pred.numel() == 0:
        return [], [], []
    scores, preds = clas_pred.max(1)
    return bbox_pred, scores, preds


def show_preds(img, output, idx, detect_thresh=0.8, classes=None, ax=None):
...
class Detection:
    ...
    def start_detection(self):
        ...
        with torch.no_grad():
            preds = self.model(self.img_temp.data.unsqueeze_(0))

        show_preds(self.img, preds, idx=0, detect_thresh=0.9, classes=self.classes)
